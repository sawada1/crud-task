        ...
        <div class="p-4">
          ...
          <ul>
            <li class="mb-2">
              ...
              <ul class="ml-6">
                <li class="text-gray-600">Web Design</li>
              </ul>
            </li>
            <li>
              <div class="flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div v-for="(taskList, column) in store.tasks" :key="column" class="bg-gray-50 p-4 border rounded"
            ...
            <div v-for="task, index in taskList" :key="index" @click="store.task = task , store.indexTask = index , store.status = column , store.showModel = true" draggable="true"
              class="bg-white dragItem p-2 rounded flex flex-col gap-2 shadow cursor-move mb-2"
              @dragstart="() => store.onDragStart(task, column , index)">
              <span class="text-[12px]"> {{ task.space }} </span>
              <span>{{ task.name }}</span>
            </div>
           {{ column }}
            <button v-tooltip.bottom="checkSpace()" @click="store.status = column , store.task = undefined , store.indexTask = undefined , store.showModel = true" class="bg-gray-200 text-sm px-4 py-2 rounded w-full">
              + Add Task
            </button>
